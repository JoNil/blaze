...
const NV_SHADER_STATE_FLAG_FRAGMENT_SHADER_SINGLE_THREADED: u8 =                        0x1;
const NV_SHADER_STATE_FLAG_STRIDE_POINT_SIZE_INCLUDED_IN_SHADED_VERTEX_DATA: u8 =       0x2;
const NV_SHADER_STATE_FLAG_ENABLE_CLIPPING: u8 =                                        0x4;
const NV_SHADER_STATE_FLAG_CLIP_COORDINATES_HEADER_INCLUDED_IN_SHADED_VERTEX_DATA: u8 = 0x8;
...
impl<'a> CommandBuilder<'a> {
    ...
    pub fn vertex_array_primitives(
        ...
        index: u32)  // Index Of First Vertex
    {
    ...
    pub fn nv_shader_state(&mut self, nv_shader_state: &NvShaderState)
    {
        ...
        self.cursor.write_u32::<LittleEndian>(nv_shader_state as *const _ as u32).unwrap();
    }

    pub fn configuration_bits(
            &mut self,
            flags8: u8,
            flags16: u16)
    {
    ...
    pub fn tile_binning_mode_configuration(
            ...
            flags: u8)
    {
        self.cursor.write_u8(CMD_TILE_BINNING_MODE_CONFIGURATION).unwrap();
        self.cursor.write_u32::<LittleEndian>(address).unwrap();
    ...
    pub fn tile_rendering_mode_configuration(
            ...
            flags: u16)
    {
        self.cursor.write_u8(CMD_TILE_RENDERING_MODE_CONFIGURATION).unwrap();
        self.cursor.write_u32::<LittleEndian>(address).unwrap();
        self.cursor.write_u16::<LittleEndian>(width).unwrap();
        self.cursor.write_u16::<LittleEndian>(height).unwrap();
            ...
            clearstencil: u8)
    {
        ...
        self.cursor.write_u32::<LittleEndian>((clearvgmask as u32 * 0x1000000) + clearzs).unwrap();
        self.cursor.write_u8(clearstencil).unwrap();
    }

    pub fn tile_coordinates(
            &mut self,
            column: u8,
            row: u8)
    {
    ...
    pub fn store_tile_buffer_general(
            &mut self,
            flags16: u16,
            flags32: u32,
            address: u32)
    {
        ...
        self.cursor.write_u16::<LittleEndian>(flags16).unwrap();
        self.cursor.write_u32::<LittleEndian>(flags32 | address).unwrap();
    }
}